...
export default function PostDetailsScreen() {
  ...
  async function handleToggleLike() {
    ...
    const alreadyLiked = currentLikes.includes(authorName);
    const newLikes = alreadyLiked
      ? currentLikes.filter((n) => n !== authorName)
      : [...currentLikes, authorName];
    setPost((prev) => (prev ? { ...prev, likes: newLikes } : prev));
    await toggleLike(post.id, authorName, alreadyLiked);
  }

  async function handleAddComment(text: string) {
    if (!post) return;
    const commentData: CommentData = { authorId: authorName, comment: text };
