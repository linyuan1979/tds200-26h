...
type PostFormProps = {
  visible: boolean;
  authorName: string;
  initialData?: PostData; // if provided, the form is in edit mode
  onClose: () => void;
  // localImages holds the URIs the camera and gallery gave us.
  // The screen that opened the form decides what to do with them.
  onSubmit: (post: PostData, localImages: string[]) => void;
};

// Parse a hashtag string like "#expo #reactnative" into chips ["expo", "reactnative"].
...
export default function PostForm({
  ...
  // When the modal opens, pre-fill the form if we are editing an existing post.
  // When it closes, the reset happens in handleClose / handleSubmit.
  useEffect(() => {
    if (visible) {
      if (initialData) {
  ...
  // Hashtags are stored as a single space-joined string (e.g. "#foo #bar"),
  // so a tag can't contain spaces itself or it would look like two tags
  // when split back apart. Collapse any spaces the user typed instead.
    ...
    >
      ...
      >
        ...
        <ScrollView style={styles.form} keyboardShouldPersistTaps="handled">
          ...
          <Text style={[styles.label, { marginTop: chips.length > 0 ? 8 : 0 }]}>
            Author
          </Text>
          <View style={styles.authorRow}>
            <Text style={styles.authorValue}>{authorName || "Anonymous"}</Text>
            {!isEditMode && <Text style={styles.authorHint}>from Sign In tab</Text>}
          </View>

          {/* Pictures — only when creating a new post, not when editing one */}
          {!isEditMode && (
            <>
